...
2. Decide mode from user intent:
   - Always check what would change first by running ingest in preview mode.
   - If the preview is clean and the user asked to build, bootstrap, update, sync, ingest, or refresh, auto-apply.
   - If the preview shows risk, ask the user before apply continues.
...
3. In a greenfield folder:
   ...
   - If the preview is clean and the user asked to build, initialize `raw/`, `pages/`, `pages/index.md`, and `log.md`, then continue.
4. Risk policy:
   - If preview shows delete operations, list the generated pages/assets to delete and ask for confirmation.
   - If preview shows rename ambiguity, manual-page protection, slug conflicts, or conversion/dependency blockers, ask before apply.
...
- preview: no files were modified and risky changes that require the LLM to ask are surfaced clearly
...
- If the run was preview only, explain whether the LLM will auto-apply next or needs to ask for confirmation.
